...
import { Task, TaskCat } from '../types';
...
export function TaskLibrary({ tasks, recentTaskIds, onArmTask }: TaskLibraryProps) {
  ...
  const recentTasks = recentTaskIds.map(id => tasks.find(t => t.id === id)).filter(Boolean) as Task[];
  ...
  return (
    <div style={{ flex: 1, overflowY: 'auto', padding: '32px 36px 40px' }}>
      ...
      <div style={{ marginBottom: 24 }}>
        <div style={{ fontSize: 22, fontWeight: 700, fontFamily: 'var(--font-display)', letterSpacing: '-.02em', marginBottom: 5 }}>Библиотека задач</div>
        <div style={{ fontSize: 14, color: 'var(--ink-3)' }}>{tasks.length} типов артефактов · выберите и AI создаст структурированный документ</div>
      </div>

      {/* Search */}
      <div style={{ position: 'relative', marginBottom: 14 }}>
        <Search size={14} color="var(--ink-3)" style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', pointerEvents: 'none' }} />
        <input
          value={query}
          ...
          style={{
            ...
            fontSize: 13.5, background: '#fff', outline: 'none', boxSizing: 'border-box',
      ...
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 24 }}>
        {CATS.map(c => {
          ...
          return (
            ...
            >
              {c.id !== 'all' && cc && (
                <div style={{ width: 6, height: 6, borderRadius: '50%', background: active ? 'rgba(255,255,255,.6)' : cc.dot }} />
              )}
              {c.label}
            </button>
          );
      ...
      {/* Recent tasks */}
      {recentTasks.length > 0 && filter === 'all' && !query && (
        <div style={{ marginBottom: 28 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 10 }}>
            <Clock size={12} color="var(--ink-3)" />
            <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--ink-3)', textTransform: 'uppercase', letterSpacing: '.07em' }}>Недавние</span>
          </div>
            ...
            {recentTasks.map(t => (
              <button
                ...
                style={{
                  ...
                  padding: '5px 11px 5px 7px', borderRadius: 999, fontSize: 12.5, fontWeight: 500,
                  border: '1.5px solid var(--border-md)', background: '#fff',
                  cursor: 'pointer', transition: 'border-color .12s, box-shadow .12s',
                }}
                onMouseEnter={e => { e.currentTarget.style.borderColor = 'var(--ai-md)'; e.currentTarget.style.boxShadow = 'var(--shadow-sm)'; }}
      ...
      {Object.entries(grouped).map(([cat, catTasks]) => {
        const cc = CAT_COLORS[cat] || { bg: 'var(--ai-lt)', color: 'var(--ai)', dot: 'var(--ai)' };
        return (
          <div key={cat} style={{ marginBottom: 28 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 10 }}>
              <div style={{ width: 8, height: 8, borderRadius: '50%', background: cc.dot, boxShadow: `0 0 0 2px ${cc.bg}` }} />
              <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--ink-3)', textTransform: 'uppercase', letterSpacing: '.07em' }}>{catLabel(cat)}</span>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))', gap: 9 }}>
              {catTasks.map(t => (
                <button
                  ...
                  onMouseEnter={e => { e.currentTarget.style.boxShadow = 'var(--shadow-md)'; e.currentTarget.style.borderColor = cc.color + '30'; e.currentTarget.style.transform = 'translateY(-1px)'; }}
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginBottom: 3 }}>
                      <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--ink)', lineHeight: 1.3 }}>{t.name}</div>
                      {t.kind === 'proto' && (
                        <span style={{ fontSize: 9.5, color: cc.color, fontWeight: 700, background: cc.bg, padding: '2px 6px', borderRadius: 4, flexShrink: 0, letterSpacing: '.02em' }}>превью</span>
                      )}
                    </div>
                    <div style={{ fontSize: 11.5, color: 'var(--ink-3)', lineHeight: 1.5 }}>{t.desc}</div>
                  </div>
                </button>
              ))}
            </div>
